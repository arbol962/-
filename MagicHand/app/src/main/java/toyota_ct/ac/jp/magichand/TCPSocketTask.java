...
/*
        ...
        try {
            ...
            //テキストフォームから送信用データの取得
            String sendTxt = ((EditText) findViewById(R.id.txtSend)).getText().toString();
            Charset.forName("UTF-8").encode(sendTxt);

            //送信したデータのサイズ確認
            System.out.println("送信に使うデータの長さは：" + sendTxt.length());
            //System.out.println(sendTxt);

            BufferedOutputStream bos = new BufferedOutputStream(socket.getOutputStream(),102400000);
            //bos.write(sendTxt.getBytes());
            bos.flush();
